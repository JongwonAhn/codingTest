...
Class12 (멀티웨이 탐색트리1)m원 탐색트리, B트리, B*,B+트리

  m원 탐색트리
- 기존 이진탐색트리의 탐색 성능을 향상시키기 위해 자식의 갯수를 m개만큼 늘린 트리
- 트리의 노드가 m개 이하의 가지를 가질수 있는 탐색트리.
- 이진 탐색 트리의 확장 된 형태이며, 탐색 트리의 제한을 따르되 2개이상(m개 이하) 자식을 갖을수 있음
- m원 탐색 트리는 서브트리의 균형에 대해서는 제한하지 않음.
- 각 노드가 자식을 많이 갖게 하여 트리의 높이를 줄이고, 전체적으로 균형을 유지한다면 탐색 성능 향상 가능.

  B트리
- m원 탐색트리를 개선한 B트리는 인덱스 구조를 구현하는데 가장 일반적으로 사용
- 모든 잎 노드는 같은 레벨에 있음
- 탐색 성능을 높이기 위해 균형 있게 높이를 유지하는 Balanced Tree


  B*트리
- 노드의 약 2/3이상이 채워지는 B트리
- 노드가 꽉 차면 분리하지 않고, 키와 포인터를 재배치하여 다른 형제 노드로 옮김.
- 삽입/삭제 시 발생하는 노드 분리는 줄이려고 고안됨.

Class13 (멀티웨이 탐색트리2) 2-3트리, 2-3-4트리, 레드블랙트리

  2-3트리
- 차수가 2또는 3인 내부 노드를 갖는 탐색 트리
- 2-노드와 3-노드만으로 구성되는 형태의 B트리
- 내부노드만 제약조건이며, 리프노드는 같은레벨이라는 조건만 갖음.

  레드블랙트리
- 레드간선과 블랙간선의 서브트리 포인터를 가짐.
- 레드간선은 2-3-4 트리의 한 노드 내에 있던 노드의 관계이고,
  블랙간선은 2-3-4 트리의 부모-자신 관계임
- 레드 블랙 트리의 탐색 방법은 보통의 이진탐색 트리의 탐색 알고리즘과 동일함.

Class14 그래프1

  그래프
- 그래프 G는 하나 이상의 정점(노드)를 포함하는 집합V와 두 정점의 쌍으로 구성되는 간선을 포함하는 집합E의 순서쌍으로 정의. G = (V,E)
- 간선은 두 정점을 연결하는 선
- 그래프는 방향성이 없는 무방향 그래프와 방향성이 있는 방향그래프가 있음.
- 완전 그래프는 모든 정점이 간선으로 서로 연결된 그래프
- 모든 트리는 그래프이다 -> 참 // 모든 그래프는 트리이다 -> 거짓(양방향그래프)
  
Class15 그래프2

  그래프탐색
- 그래프에서 특정 정점을 찾는 기본 연산
- 그래프 순회에는 깊이 우선 탐색(DFS)와 너비 우선 탐색(BFS) 가 있음 

  깊이우선탐색
- 출발점v 를 방문하는 것으로 시작 하여 다음으로 v에 인접하고 아직 방문하지 않은 정점w를 선택하여 w를 출발점으로 다시 깊이 우선 탐색 시작.
- 두 과정을 모든 정점을 한 번씩 방문할 때 까지 반복. Stack사용

  너비우선탐색
- 출발점v를 방문하는 것으로 시작 하여 다음으로 v에 인접한 정점 w를 모두 방문후, 다시 w에 인접한 방문하지 않은 정점들을 차례로 방문.
- 너비 우선 탐색은 인접한 정점을 모두 방문하기 때문에 스택이 필요하지 않고 대신 큐를 사용

  최소 신장 트리
- 신장 트리란 그래프 G의 모든 정점과 간선의 일부(또는 전부)를 포함하는 트리.
- 그래프G의 부분 그래프 중에서 간선의 수가 가장 작은것이 최소 신장 트리.
- 최소 신장트리를 구하는 알고리즘은 솔린 알고리즘, 프림 알고리즘, 크루스칼 알고리즘이 있다

  선형자료구조
- 배열, 스택, 큐, 연결리스트

  트리
- 스레드 트리, 힢, 선택트리, 이진트리, 이진탐색, Splay, AVL, BB트리, 멀티웨이 탐색트리, 그래프